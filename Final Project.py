...
# Creating a table
cursor.execute("CREATE TABLE laptops( Name varchar(80), CPU varchar(50), RAM varchar(50), STORAGE varchar(50), VGA varchar(50), DISPLAY varchar(50), Price int , PRIMARY KEY (Name) )")
dbc.commit()
...
for page in range(1,41):
    page = str(page)
    url = "https://adak.shop/product-category/laptop-and-accessories/laptop/page/" + page + "/"
    ...
    for laptop in twelve:
        ...
        if re.match(r'.*<bdi>(\d+,*\d*,*\d*)' ,str(price1) ) != None:
...
test = np.array(['Core i3 1215U' ,'16GB DDR4','1TB SSD','4GB GTX 1650','15.6" FullHD 144Hz'])
print("Laptop with these specs:\nCPU: %s \nRAM: %s \nHardDisk: %s \nVGA: %s \nDisplay: %s \n" %(test[0], test[1], test[2], test[3], test[4]))
new_data = le.fit_transform(test)
shape = new_data.reshape(1, -1)

# Mr. programmer, gimme the knowledge of this computer for forseeing the price of my customized laptop!
